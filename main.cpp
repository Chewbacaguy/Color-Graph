...
//nohaya adjacencia con alguien previamente coloreado
...
// Aquí va tu código
  ...
  for(int a=0; a<n; a++){
    ...
    if(!colored[i]){
      colored[i] = ++colNum;
      unordered_set<int> conj; // conjunto vrtx con el mismo color
      conj.insert(i); // inserto al primer vertice de eses color

      for(int b = a+1; b<n; b++){
        int j= vtxColor[b].vtx;
        if(!colored[j] && !matAdj[i][j] && canColor(matAdj, conj, j)){
          colored[j] = colNum;
          conj.insert(j); // inserto al primer vertice de eses color
